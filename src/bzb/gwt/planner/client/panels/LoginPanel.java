...
import bzb.gwt.planner.client.CUser.GENDER;
...
public class LoginPanel extends FlowPanel {
	 ...
	 * service.
	 */
	private final OpenIdServiceAsync openidService = GWT
			.create(OpenIdService.class);
			...
			openidService.verifyAuth(Window.Location.getHref(),
					new AsyncCallback<CUser>() {
						...
						public void onSuccess(CUser user) {
							...
							if (Planner.getUser().isRegistered()) {
								...
								if (user.getAge() >= AGEBOX_YEAR_ZERO) {
									ageBox.setSelectedIndex(user.getAge() - AGEBOX_YEAR_ZERO);
								} else {
								...
								if (user.getUsername() != null) {
									emailBox.setVisibleLength(user.getUsername().length() + 5);
								...
								for (final CUser.GENDER g : CUser.GENDER.values()) {
									genderBox.addItem(g.toString());
								}
								if (user.getGender() != null) {
									genderBox.setSelectedIndex(user.getGender().ordinal());
								}
							    ...
							    Button done = new Button();
									...
									public void onClick(ClickEvent event) {
										...
										Planner.getUser().setGender(GENDER.valueOf(genderBox.getItemText(genderBox.getSelectedIndex())));
